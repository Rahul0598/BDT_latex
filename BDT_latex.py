...
def find_words(list_w, color, text):
    ...
    text = ' ' + text + ' '
    for word in list_w:
        p_w = re.compile(r'\s+' + word + r'[\s\W]', re.IGNORECASE)
        ...
        for match_str in m_w:
            ...
            if m_word is not None:  # Add check to avoid NoneType errors
...
def find_end_words(list_wildcards, color, text):
    ...
    for wild_c in list_wildcards:
        p_w = re.compile(r'\s+\w+' + wild_c + r'[\W\s]', re.IGNORECASE)
        m_w = p_w.finditer(text)
        for match_str in m_w:
            match = match_str.group()
            # we have to find the match word, to allow upper case
            p_word = re.compile(r'\w+' + wild_c, re.IGNORECASE)
            m_word = p_word.search(match)
            if m_word is not None:  # Add check to avoid NoneType errors
                new_word = color_code(m_word.group(), match, color)
                text = text.replace(match, new_word)
...
def open_file(name, mod):
    ...
    try:
        file_obj = open(file_name, mod, encoding='utf-8')  # Add encoding parameter
    except Exception as e:  # Use Exception as e pattern
...
def check_recursion(line, global_path, p):
    ...
    if len(m) > 0:
        for file_x in m:
            ...
            if m_file is not None:  # Add check to avoid NoneType errors
...
def diagnostic_test(name):
    ...
    # read the content of the file, process and write them into the output
    line = tex_in_file.readline()
    while (line != ''):
        # check if this line is a comment
        m0 = p5.match(line)
        if m0 is None:
            # check if this line is the beginning of the document
            m1 = p1.match(line)
            if m1 is not None:
...
p1 = re.compile(r'\s*\\begin\s*\{document\}')
# insertion of files 
p2 = re.compile(r'\s*\\include\s*\{[^\\\\]*\}')
p3 = re.compile(r'\s*\\input\s*\{[^\\\\]*\}')
p4 = re.compile(r'\s*\\subfile\s*\{[^\\\\]*\}')
# comments
p5 = re.compile(r'\s*\%')
# path of a file
p6 = re.compile(r'~?[\./\w\_\-\:\\\\]*[/\\\\]')
...
p_node = re.compile(r'\s*\\node')
# color's commands
p_color_command = re.compile(r"\\BDT(Red|Blue|Purple|Orange|Green|Brown)\s*\{[^\s]*\}")
p_color_word = re.compile(r'\{.*\}')

# define the set of actions to make in the text
set_actions = set()
for i in ['r', 'b', 'p', 'o', 'g', 'B']:
    set_actions.add(i)  # Use add() method instead of update() for single characters

actions = set()
tex_file = ''

if len(input_args) > 1 and input_args[1][0] == '-':
    try:
        tex_file = input_args[2]
    except IndexError:
        print('.tex file not found.')
        sys.exit(1)  # Use sys.exit instead of exit
    
    if input_args[1] == '--undo':
        undo_diagnostic_test(tex_file)
        sys.exit(0)  # Add proper exit code
    else:
        len_a = len(input_args[1])
        for i in input_args[1][1:len_a]:
            if i in set_actions:
                actions.add(i)  # Use add() instead of update for single characters
            else:
                print(f'Parameter {i} is not part of the available actions')  # Use f-string instead of % formatting
                sys.exit(1)  # Use sys.exit instead of exit
